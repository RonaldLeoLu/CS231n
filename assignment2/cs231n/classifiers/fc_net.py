...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        drelu1, dW2, db2 = affine_backward(dSoft, fc2_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            fc_1, fc_cache[str(i+1)] = affine_forward(x, self.params['W'+str(i+1)], self.params['b'+str(i+1)])

            if self.use_batchnorm:
                bn_1, bn_cache[str(i+1)] = batchnorm_forward(fc_1, self.params['gamma' + str(i+1)], 
                    self.params['beta' + str(i+1)], self.bn_params[i])
                relu_1, relu_cache[str(i+1)] = relu_forward(bn_1)
            else:
                relu_1, relu_cache[str(i+1)] = relu_forward(fc_1)

            if self.use_dropout:
                dt_1, dt_cache[str(i+1)] = dropout_forward(relu_1, self.dropout_param)
                x = dt_1.copy()
            else:
                x = relu_1.copy()

        scores, fc_cache[self.num_layers] = affine_forward(x, self.params['W'+str(self.num_layers)], 
        ...
        dx, dw_last, db_last = affine_backward(dSoft, fc_cache[self.num_layers])
        grads['W'+str(self.num_layers)] = dw_last + self.reg * self.params['W'+str(self.num_layers)]
        grads['b'+str(self.num_layers)] = db_last

        for i in range(self.num_layers-1, 0, -1):
            if self.use_dropout:
                ddt = dropout_backward(dx, dt_cache[str(i)])
                drelu = relu_backward(ddt, relu_cache[str(i)])
            else:
                drelu = relu_backward(dx, relu_cache[str(i)])

            if self.use_batchnorm:
                dbnout, dgamma, dbeta = batchnorm_backward(drelu, bn_cache[str(i)])
                dx, dw, db = affine_backward(dbnout, fc_cache[str(i)])
                grads['gamma' + str(i)] = dgamma
                grads['beta' + str(i)] = dbeta
            else:
                dx, dw, db = affine_backward(drelu, fc_cache[str(i)])
