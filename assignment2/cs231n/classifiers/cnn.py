...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        mp,mp_cache = max_pool_forward_fast(relu1, pool_param)
        fc1, fc1_cache = affine_forward(mp, W2, b2)
        relu2, relu2_cache = relu_forward(fc1)
        fc2, fc2_cache = affine_forward(relu2, W3, b3)
        scores = fc2.copy()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dfc2, dW3, db3 = affine_backward(dsoft, fc2_cache)
        drelu2 = relu_backward(dfc2, relu2_cache)
        dfc1, dW2, db2 = affine_backward(drelu2, fc1_cache)
        dmp = max_pool_backward_fast(dfc1, mp_cache)
        drelu1 = relu_backward(dmp, relu1_cache)
        dconv, dW1, db1 = conv_backward_fast(drelu1, conv_cache)
